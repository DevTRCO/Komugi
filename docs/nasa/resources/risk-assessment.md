...
## Purpose
Identify all failure modes, edge cases, error propagation paths, and data integrity risks in the target code. Think like a failure analyst — your job is to find every way this code can break.
...
- [ ] API routes return appropriate HTTP status codes for each error type
...
- [ ] Optimistic concurrency control exists for conflict-prone operations
...
For each critical path in the code, answer:
...
- Monitoring/alerting would catch this in production

## Severity Classification

| Finding | Grade |
|---------|-------|
| Unhandled promise rejection in payment flow | CRITICAL |
...
| Race condition in subscription check | CRITICAL |
